...
EVENT_ITEM(NVTEVT_KEY_CUSTOM1,UIFlowWndPhoto_OnKeyShutter2)
#else
EVENT_ITEM(NVTEVT_KEY_CUSTOM1,UIFlowWndPhoto_OnCustom1)
...
static void UIFlowWndPhoto_OnExeSetImageRatio(void)
{
...
{
    #if (_FD_FUNC_==0&&_SD_FUNC_==0)
    SysSetFlag(FL_FD,FD_OFF);
    #endif
...
void UIFlowDCF_TimelapseRecUpdateName(void)
{
    CHAR FolderName[6]=FILEDB_TMLPS;//"TMLPS";
    CHAR FileName[5]="TIME";
    ...
    if (strncmp(DCFpathname, FolderName, 5))//if no this folder
    {
...
INT32 UIFlowWndPhoto_OnBattery(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
 static volatile BOOL bBatteryOn = FALSE;
    ...
    if (KeyScan_IsACIn())
    {
        ...
        if (bBatteryOn==FALSE)
            UxState_SetData(&UIFlowWndPhoto_StatusICN_BatteryCtrl,STATE_CURITEM,UIFlowWndPhoto_StatusICN_Battery_ICON_TRANSPAENT);
...
INT32 UIFlowWndPhoto_OnCustom1(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	||(_MODEL_DSC_ == _MODEL_DUAL_QIZHENG_M1_)||(_MODEL_DSC_ == _MODEL_CARDV_GT200_)\
...
INT32 UIFlowWndPhoto_PNL_FDFrame_OnRedraw(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    prectOne = &rectOne;
    static FD_FACE   FdInfo[FD_NUM_MAX];
    PDISP_OBJ        pDispObj = NULL;
    static DISPLAYER_PARAM  DispLyr = {0} ;

    Fdnumber = FD_GetRsltFace(FdInfo, &pPhotoExeInfo->FdDispCoord);
    ...
    DBG_IND("[fd]FDFrame_OnDraw color=%d, Fdnumber =%d\r\n",uiRectColor,Fdnumber);
    ...
    if(uiRectColor != _OSD_INDEX_TRANSPART)
    {
        for (i=0;i<Fdnumber;i++)
        {
            ...
            prectOne->x = FdInfo[i].x;
            prectOne->y = FdInfo[i].y;
            prectOne->w = FdInfo[i].w;
            prectOne->h = FdInfo[i].h;
            ...
            DispLyr.SEL.SET_FDSIZE.uiFDX = prectOne->x;
            DispLyr.SEL.SET_FDSIZE.uiFDY = prectOne->y;
            DispLyr.SEL.SET_FDSIZE.uiFDW = prectOne->w;
            DispLyr.SEL.SET_FDSIZE.uiFDH = prectOne->h;
            DispLyr.SEL.SET_FDSIZE.uiFDBORD_W = 2;
            DispLyr.SEL.SET_FDSIZE.uiFDBORD_H = 2;
            pDispObj->dispLyrCtrl(DISPLAYER_FD, DISPLAYER_OP_SET_FDSIZE, &DispLyr);

            DispLyr.SEL.SET_FDCOLOR.FD_NUM = DISPFD_NUM0<<i;
            DispLyr.SEL.SET_FDCOLOR.uiFDCRY = 0x72;
            DispLyr.SEL.SET_FDCOLOR.uiFDCRCb = 0xCD;
            DispLyr.SEL.SET_FDCOLOR.uiFDCRCr = 0x2C;
