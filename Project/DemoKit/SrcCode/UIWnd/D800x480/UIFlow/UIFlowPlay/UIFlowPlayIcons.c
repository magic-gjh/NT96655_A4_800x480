...
//    CHAR    chaFullName[34] = { 0 };
    ...
    snprintf(item1_Buf,20,"%02d/%02d/%02d",modDateTime[0],modDateTime[1],modDateTime[2]);
...
void FlowPB_IconDrawMovTotalTime(BOOL bShow)
{
    static char item1_Buf[32];
    UINT32 uiTotalTimeSecs;
	
    if (bShow==FALSE)
    {
        UxCtrl_SetShow(&UIFlowWndPlay_StaticTXT_MovTotalTimeCtrl, FALSE);
        return;
    }
    
    uiTotalTimeSecs=UIMovObj_GetDataPlayMovie(PLAYMOVIE_TOTAL_SECOND);
    debug_msg("uiTotalTimeSecs:%d..\r\n",uiTotalTimeSecs);	
    snprintf(item1_Buf, 32, "%02ld:%02ld:%02ld", uiTotalTimeSecs/3600, (uiTotalTimeSecs%3600)/60, (uiTotalTimeSecs%3600)%60);

    UxStatic_SetData(&UIFlowWndPlay_StaticTXT_MovTotalTimeCtrl,STATIC_VALUE,Txt_Pointer(item1_Buf));

    UxCtrl_SetShow(&UIFlowWndPlay_StaticTXT_MovTotalTimeCtrl, TRUE);
