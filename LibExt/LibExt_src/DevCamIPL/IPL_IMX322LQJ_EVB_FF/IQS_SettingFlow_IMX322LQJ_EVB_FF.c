#ifndef _IQS_SETTINGFLOW_IMX322LQJ_EVB_FF_C
#define _IQS_SETTINGFLOW_IMX322LQJ_EVB_FF_C
...
static char     IQS_IMX322_VerInfo[]    = "IQS_A7_20150417";
static char     IQS_IMX322_BuildDate[]  = __DATE__ ", " __TIME__;
...
static ER IPE_IQparam(IQS_FLOW_MSG MsgID, IPE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    if ( IQS_GetIsoRange( MsgID, &iso, &isoL, &isoH, &ISO_IdxL, &ISO_IdxH ) == TRUE )
    {
        ...
        IQ_YCon = IQS_Intpl((int)iso, (int)Contrast[ISO_IdxL][0], (int)Contrast[ISO_IdxH][0], (int)isoL, (int)isoH);
        IQ_CCon = IQS_Intpl((int)iso, (int)Contrast[ISO_IdxL][1], (int)Contrast[ISO_IdxH][1], (int)isoL, (int)isoH);
    }
    else
    {
        IQ_CCon = IQ_YCon = 0;
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PREIME_D2D_IPE_QV:
            ...
            IQ_IPE_YCCcon.YCon   = ColorEffectTable[IMEffect_Value].YCon + IQ_YCon;
            IQ_IPE_YCCcon.UVCon  = ColorEffectTable[IMEffect_Value].CCon + IQ_CCon;
...
ER IQCB_SettingFlowFunc(IQS_FLOW_MSG MsgID, ENG_SUBFUNC *Info, IPL_HAL_GROUP* group)
{
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PRV_SIE_INIT:
            IQS_SetVerInfo(IQS_IMX322_VerInfo);
            IQS_SetBuildDate(IQS_IMX322_BuildDate);
            break;
